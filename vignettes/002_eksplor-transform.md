...
``` r
dim(iris) # return atau set sebuah dimensi sebuah object (ada berapa kolom dan row)
```

    ## [1] 150   5

``` r
str(iris) # menampilkan struktur internal object R (detail setiap kolom seperti value dan penjelasan)
```
...
``` r
colnames(iris) # menampilkan semua colomn name
```
...
``` r
head(iris) # menampilkan 6 baris pertama
```
...
``` r
tail(iris) # menampilkan 6 baris terakhir
```
...
``` r
summary(iris) # menampilkan info seperti median,min,max,mean setiap kolom
```
...
``` r
class(iris$Species) # menampilkan tipe data sepcies
```

    ## [1] "factor"

``` r
length(iris$Petal.Width) # menampilkan jumlah kolom "petal.width" di tabel "iris"
```

    ## [1] 150

``` r
levels(iris$Species) # menampilkan value apa saja yang ada di kolom species di table iris
```

    ## [1] "setosa"     "versicolor" "virginica"

``` r
length(levels(iris$Species)) # menampilkan jumlah level yang ada di kolom species di table iris
```

    ## [1] 3

``` r
unique(iris$Species) # menampilkan value unik dan duplikat di hilangkan
```
...
``` r
length(unique(iris$Species)) # menampilkan jumlah value unik di kolom species di table iris
```

    ## [1] 3

``` r
mean(iris$Sepal.Length) # kalkulasi mean kolom sepal.length
```

    ## [1] 5.843333

``` r
sd(iris$Sepal.Length) # kalkulasi standard deviation sepal.length
```

    ## [1] 0.8280661

``` r
median(iris$Sepal.Width) # kalkulasi median kolom sepal.width
```

    ## [1] 3

``` r
sum(iris$Petal.Length) # menjumlahkan total nilai petal.length
```

    ## [1] 563.7

``` r
cor(iris[, -5]) #kalkulasi variansis antar kolom
```
...
``` r
vis_dat(typical_data) #menampilkan grafik yang menunjukan kolom serta tipe datanya
```

<img src="002_eksplor-transform_files/figure-gfm/explore-iris2-1.png" style="display: block; margin: auto;" />

``` r
skim(typical_data) #dicetak di console dan tulisan, bukan grafik
```
...
1.  (select) select kolom year,month,wind, preassure dari dataset storm
2.  (filter) memilih barus dengan tahun antara 2000 hingga 2015
3.  (mutate) mengubah nilai dari kolom month yang semula angka menjajdii
    nama bulan
4.  (group\_by) melakukan grouping berdasarkan kolom month
5.  (summarise) menghitung rerata kecepatan angin dan rerata tekana
    udara
